<?php
    ...
    class Anuncios extends model {
        ...
        public function getMeusAnuncios() {
            ...
            $sql= $this->db->prepare("SELECT *,(select anuncios_imagens.urli
        ...
        public function getUltimosAnuncios($page,$perPage) {
            ...
            $sql= $this->db->query("SELECT *,(select anuncios_imagens.urli
        ...
        public function setAnuncio($titulo, $valor, $descricao, $estado, $categoria,$id_usuario) {
            $sql= $this->db->prepare("INSERT INTO anuncios SET id_categorias= :categoria, 
        ...
        public function deleteAnuncio($id) {
            ...
            $sql= $this->db->prepare("DELETE FROM anuncios_imagens
             WHERE id_anuncio= :id");
            $sql->bindValue(":id", $id);
            $sql->execute();
        ...
        public function getInfo($id) {
            $array= array();
            $sql= $this->db->prepare("SELECT *,(select usuarios.telefone from usuarios where usuarios.id = anuncios.id_usuario )as user FROM anuncios WHERE id= :id");
            ...
            $array['fotos']= array();
            $sql= $this->db->prepare("SELECT id,urli 
        ...
        public function editAnuncio($titulo, $valor, $descricao, $estado, $categoria, $fotos,$id) {
            $array= array();
            $sql= $this->db->prepare("UPDATE anuncios SET id_categorias= :categoria,id_usuario= :id_usuario,
            ...
            if (count($fotos)> 0) {
                for($q=0;$q<count($fotos['tmp_name']);$q++) {
                    ...
                    if (in_array($tipo,array('image/jpeg','image/png'))) {
                        $tmpname= time().'.jpg';
                        move_uploaded_file($fotos['tmp_name'][$q],''.BASE_URL.'assets/images/anuncios/'.$tmpname);
